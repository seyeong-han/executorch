...
def _run_flatc(args: Sequence[str]) -> None:
    ...
    if importlib.resources.is_resource(__package__, _FLATC_RESOURCE_NAME):
        ...
        with importlib.resources.path(__package__, _FLATC_RESOURCE_NAME) as flatc_path:
            try:
                subprocess.run(
                    [flatc_path] + list(args), check=True, capture_output=True
                )
            except subprocess.CalledProcessError as e:
                print("CAUGHT FLATC EXCEPTION", flush=True)
                print(f"FLATC STDOUT: {e.stdout.decode(errors='replace')}", flush=True)
                print(f"FLATC STDERR: {e.stderr.decode(errors='replace')}", flush=True)
                raise
    else:
